...
class Exercise1Test(LiveServerTestCase):
    def test_media_serving(self):
        ...
        self.assertEqual(hw_content, resp.read())

class Exercise2Test(LiveServerTestCase):
    def test_media_serving(self):
        """ Test that out test.txt file can be retrieved using the media URL. """
        media_root = settings.MEDIA_ROOT
        self.assertEqual(settings.MEDIA_URL, "/media/")
    ...
    def test_media_example_view(self):
        """ Test the media example view is set up correctly 
        and contains a link to test.txt file in the template it renders.
        """
        c = Client()
